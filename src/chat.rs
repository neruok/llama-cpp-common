...
//!
//! Unlike `LlamaModel::apply_chat_template`, which detects a template from a
//! fixed list of known templates, this module renders the chat template with
//! llama.cpp's full Jinja engine. It handles arbitrary templates, including
//! loops and the `add_generation_prompt` branch, and it can render a literal
//! template without loading a model.
...
impl LlamaMinjaChatTemplate {
    ...
    pub fn from_model(model: &LlamaModel) -> Result<Self, MinjaChatTemplateError> {
        ...
        let handle = unsafe { sys::llama_rs_chat_template_init(model.as_ptr().cast_const(), ptr::null()) };
        NonNull::new(handle)
    ...
    pub fn new(template: &LlamaChatTemplate) -> Result<Self, MinjaChatTemplateError> {
        ...
        let handle =
            unsafe { sys::llama_rs_chat_template_init(ptr::null(), template.as_c_str().as_ptr()) };
        NonNull::new(handle)
            .map(|templates| Self { templates })
            .ok_or(MinjaChatTemplateError::InitFailed)
    }
    ...
    ///
    /// `enable_thinking` is forwarded to the template as its `enable_thinking`
    /// variable and is ignored by templates that do not use it.
    ///
    /// # Errors
    ///
    ...
    ///
    /// Each value is JSON text, which is the type llama.cpp stores, so a caller
    /// passes `("reasoning_effort", "\"high\"")` for the string `high`. A
    /// template that does not read a key ignores it, and an absent key leaves
    /// the template's own default in place.
    ///
    /// # Errors
    ///
    /// [`MinjaChatTemplateError::NulByte`] when a name or a value contains an
    /// interior null byte, and otherwise as for [`LlamaMinjaChatTemplate::render`].
    pub fn render_with_kwargs(
        ...
        let status = unsafe {
            sys::llama_rs_chat_template_apply(
        ...
        if status != sys::LLAMA_RS_STATUS_OK || out_prompt.is_null() {
            return Err(MinjaChatTemplateError::ApplyFailed(status));
        }
        ...
        unsafe { sys::llama_rs_string_free(out_prompt) };
        prompt
    }
}
...
impl Drop for LlamaMinjaChatTemplate {
    fn drop(&mut self) {
        unsafe { sys::llama_rs_chat_template_free(self.templates.as_ptr()) }
    }
}
