...
///
/// This wrapper binds llama.cpp's speculative state to sequence 0. Batches
/// passed to [`Self::process`] must therefore contain only sequence 0.
#[derive(Debug)]
...
impl<'model> MtpSpeculative<'model> {
    ...
    pub fn new(
        ...
        let raw = unsafe {
            sys::llama_rs_mtp_speculative_init(
                target_context.as_ptr(),
    ...
    pub fn begin(&mut self, prompt_tokens: &[LlamaToken]) -> Result<(), MtpSpeculativeError> {
        ...
        let status = unsafe {
            sys::llama_rs_mtp_speculative_begin(self.raw.as_ptr(), prompt.as_ptr(), prompt.len())
        };
    ...
    pub fn process(&mut self, batch: &LlamaBatch<'_>) -> Result<(), MtpSpeculativeError> {
        let status =
            unsafe { sys::llama_rs_mtp_speculative_process(self.raw.as_ptr(), batch.as_ptr()) };
        status_to_result(status)
    }
    ...
    pub fn draft(
        ...
        let status = unsafe {
            sys::llama_rs_mtp_speculative_draft(
                self.raw.as_ptr(),
                n_past,
                id_last.0,
                prompt.as_ptr(),
        ...
        if status == sys::LLAMA_RS_STATUS_ALLOCATION_FAILED {
    ...
    pub fn accept(&mut self, n_accepted: u16) -> Result<(), MtpSpeculativeError> {
        let status =
            unsafe { sys::llama_rs_mtp_speculative_accept(self.raw.as_ptr(), n_accepted) };
        status_to_result(status)
    }
}
...
impl Drop for MtpSpeculative<'_> {
    fn drop(&mut self) {
        unsafe {
            sys::llama_rs_mtp_speculative_free(self.raw.as_ptr());
        }
    }
}
...
fn status_to_result(status: sys::llama_rs_status) -> Result<(), MtpSpeculativeError> {
